    ...
    override fun onLocationChanged(location: Location) {
        ...
        if (currentLocationMarker == null) {
            currentLocationMarker = Marker(map).apply {
                ...
                title = "Mi Ubicación"
                icon = ContextCompat.getDrawable(this@OtherUserMapActivity, R.drawable.baseline_location_pin_24)?.let { drawable ->
    ...
    private fun setJsonLocations() {
        for (location in jsonLocations) {
            ...
            val marker = Marker(map).apply {
                ...
                icon = ContextCompat.getDrawable(this@OtherUserMapActivity, R.drawable.baseline_museum_24)?.let { drawable ->
                    vectorToBitmap(drawable)
                }?.let { bitmap ->
                    BitmapDrawable(resources, bitmap)
                }
